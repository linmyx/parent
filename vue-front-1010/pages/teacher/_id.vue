      ...
      <section class="mt30">
        <div>
          ...
          <article class="comm-course-list">
            <ul class="of" v-show="courseList.length > 0">
...
export default {
  ...
  //     // this.getTeacherCourseInfo()
  // },
    // methods:{
    //   getTeacherCourseInfo(){
    //     teacherApi.getTeacherCourseInfo(this.teacherId)
    //     .then(response =>{
    //         this.teacherCourseList=response.data.data
    //     })
    //   }
    // }
    asyncData({ params, error }) {
        return teacherApi.getTeacherCourseInfo(params.id).then(response => {
          return { 
            teacher: response.data.data.teacher,
            courseList: response.data.data.course
          }
        })
      }
  };
