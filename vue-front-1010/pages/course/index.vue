          ...
          <dl>
            ...
            <dd class="c-s-dl-li">
              <ul class="clearfix">
                ...
                <li class="current:" v-for="(item,index) in subjectNestedList" :key="index" :class="{active:oneIndex==index}" >
...
import courseApi from '@/api/course';
  export default {
      ...
      methods: {
         //1.查询第一页的数据
        initCourseFirst(){
          courseApi.getCoursePage(1,8,this.searchObj)
          .then(response =>{
        ...
        gotoPage(page){
          courseApi.getCoursePage(page,8,this.searchObj)
          .then(response =>{
            this.data=response.data.data
          })
        },
        ...
        searchOne(id,index){
          ...
          this.gotoPage(1)
             for(let i =0;i<this.subjectNestedList.length; i++){
                 var oneSub= this.subjectNestedList[i]
                 if(id === oneSub.id){
                      this.subSubjectList=oneSub.children
                 }
             }
        },
        searchTwo(id,index){
