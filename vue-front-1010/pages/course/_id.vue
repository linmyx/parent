                  ...
                  <section class="mt20">
                    <div class="lh-menu-wrap">
                      <menu id="lh-menu" class="lh-menu mt10 mr10">
                        <ul>
                          <!-- 文件目录 -->
                          <li class="lh-menu-stair" v-for="chapter in video" :key="chapter.id">
                            ...
                            <ol class="lh-menu-ol" style="display: block;">
                              <li class="lh-menu-second ml30" v-for="video in chapter.children" :key="video.id">
                                <a :href="'/player/'+video.videoSourceId" target="_blank">
                                  <span class="fr">
                                    <i class="free-icon vam mr10">{{video.isFree == true ?"免费试听" : ""}}</i>
                                  </span>
            ...
            <div class="of">
              <section class="n-reply-wrap">
                ...
                <p class="of mt5 tar pl10 pr10">
                  ...
                  <input type="button" @click="addComments()" value="回复" class="lh-reply-btn">
                </p>
              </section>
            </div>
          </li>
...
import comment from '@/api/commont'
...
export default {
  ...
  mounted() {
    this.loginInfo=cookie.get("userInfo")
    if(this.loginInfo){
      this.user=JSON.parse(this.loginInfo)
    }
  },
  methods: {
    ...
    addComments(){
      var loginInfo= cookie.get("userInfo")
    ...
    initComment(){
      comment.getComment(this.page,this.limit,this.courseId)
      .then(response => {
        this.data=response.data.data
      })
    },
    gotoPage(page){
      comment.getComment(page,this.limit,this.courseId)
      .then(response => {
        this.data=response.data.data
      })
    },
    createOrder(){
      ordersApi.addOrders(this.courseId)
      .then(response => {
         //获取订单号
         //生成订单之后，跳转订单显示页面
